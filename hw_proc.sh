...
while (( $i < ${#array_pids[@]} ))
        do
           if [[ -d $( echo /proc/${array_pids[$i]}) ]]
            ...
            vtime=$(( utime + stime ))
            time=$(date -d@$(( vtime / clk_tck  )) +%M:%S) #<---- Результирующее поле, содержащее время в формате (Min:Sec), если нужно ещё и часы, то добавиить в вывод ещё одно значение (%H)
            #/
            ...
            #Используются ранее сформированные массивы array_pts и array_tty
                    if [[ $(echo $tty) -eq 0  ]]
                        then
                                tty='?'
